...
export const ModelVariantsPlugin: Plugin = async (input) => {
  async function refreshVariantsCache() {
    try {
      ...
      // Atomic write: write to unique .tmp then rename. Unique suffix avoids
      // races when the plugin is loaded concurrently (server + reload).
      // rename() is atomic on POSIX, so concurrent readers never see partial JSON.
      // Always write — even when empty — to avoid stale cache from a previous run.
