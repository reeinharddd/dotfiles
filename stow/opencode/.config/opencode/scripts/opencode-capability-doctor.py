...
def check_omo_routes(config: Path) -> list[str]:
    ...
    errors = []
    native = [
        f"{where}={model}" for where, model in routes if model.startswith("opencode/")
    ]
    paid_zen = [
        f"{where}={model}"
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", default=str(Path.home() / ".config/opencode/opencode.jsonc")
    )
    ...
    missing_env = [
        name for name in ENV_KEYS if name not in os.environ and name not in env_names
    ]
    if missing_env:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            checks["agents"] = {
                name: bool(re.search(rf'"{re.escape(name)}"\s*:\s*\{{', source))
                for name in REQUIRED_AGENTS
            }
            checks["mcps"] = {
                name: bool(
                    re.search(
                        rf'"{re.escape(name)}"\s*:\s*\{{[\s\S]*?"enabled"\s*:\s*true',
                        source,
                    )
                )
            ...
            checks["skillRoots"] = (
                [str(config.parent / "plugins/bodega-global-skills.json")]
                if (config.parent / "plugins/bodega-global-skills.json").exists()
                else []
            )
            missing_agents = [
                name for name, present in checks["agents"].items() if not present
            ]
            missing_mcps = [
                name for name, enabled in checks["mcps"].items() if not enabled
            ]
            if missing_agents:
                errors.append("required agents missing: " + ", ".join(missing_agents))
            if missing_mcps:
                errors.append(
                    "required MCPs disabled/missing: " + ", ".join(missing_mcps)
                )
